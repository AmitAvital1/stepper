...
public class AdminHistoryController implements AdminBodyControllerDefinition {
    ...
    private ObservableList<tableRow> rowList = FXCollections.observableArrayList();
    ...
    public void show() {
        ...
        HistoryRefresher historyRefresher = new HistoryRefresher();
        timer = new Timer();
        timer.schedule(historyRefresher, 0, 1000);
        bodyForHistoryExecutionController.setTimer(timer);

    }
    private class HistoryRefresher extends TimerTask {
...
//                        );
                        ...
                        if(historyOfExecutionsFlowsTable.getSelectionModel().getSelectedItem() != null)
                            selectedItem = historyOfExecutionsFlowsTable.getSelectionModel().getSelectedItem().theFlow.getUniqueId();

                        sortTable(historyOfExecutionsFlowsTable,sortTableChoice.getValue(),list);
                        historyOfExecutionsFlowsTable.sort();

                        if (selectedItem != null) {
                            String uniq = selectedItem;
                            tableRow newSelect = historyOfExecutionsFlowsTable.getItems().stream().filter(row -> row.theFlow.getUniqueId().equals(uniq)).findFirst().get(); // Restore selection
                            historyOfExecutionsFlowsTable.getSelectionModel().select(newSelect);
                        }
                        ...
                        sortTableChoice.setOnAction(event -> sortTable(historyOfExecutionsFlowsTable,sortTableChoice.getValue(), list));
                    });
                }
            }
        });
    ...
    private void sortTable(TableView<tableRow> historyOfExecutionsFlowsTable, String value, List<tableRow> list) {
        rowList.clear();
        rowList.addAll(list);
        ...
        historyOfExecutionsFlowsTable.getItems().removeIf(filterPredicate.negate());
    }
